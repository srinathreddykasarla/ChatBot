...
def create_datasets(directory, from_train, to_train, from_test, to_test):
    print("Creating dataSets from %s" % (directory))
    with gfile.GFile(directory+'/movie_lines.txt', mode="rb") as f:
        lines = {}
        for line in f:
            words = line.split(" +++$+++ ")
    ...
    count = 0
    data = []
    with gfile.GFile(directory+'/movie_conversations.txt', mode="rb") as f:
        for line in f:
            words = line.split(" +++$+++ ")
            lis = eval(words[-1])
            for i in xrange(len(lis)-1):
                count+=1
                data.append((lines[lis[i]],lines[lis[i+1]]))
    
    with gfile.GFile(from_train+"", mode="w") as g:
        with gfile.GFile(to_train+"", mode="w") as h:
            for c in xrange(0,count):   
                if c%100!=0:          
                    g.write("".join(data[c][0]))
                    h.write("".join(data[c][1]))

    with gfile.GFile(from_test+"", mode="w") as g:
        with gfile.GFile(to_test+"", mode="w") as h:
            for c in xrange(0,count):
                if c%100==0:
                    g.write("".join(data[c][0]))
                    h.write("".join(data[c][1]))
